...
{
    [ApiVersion("2.0")]   // used for triggering different versions using http
    [ApiController]
    ...
    //[Route("api/v{v:apiVersion}/[controller]")]  // this we use when we want to trigger different versions through url (we should comment the line wch triggeres different version through http). In {v:apiVersion}, apiVersion will be substituted by the value written in [ApiVersion("2.0")] and also we should change the url in the postman to https://localhost:7127/api/v2.0/shirts/ 
    
    // another method is versioning with query string : in this method we should comment [Route("api/v{v:apiVersion}/[controller]")] line and uncomment [Route("api/[controller]")] line. And in portman url, we should use this url: https://localhost:7103/api/shirts?api-version=2.0 for version 2 and https://localhost:7103/api/shirts?api-version=1.0  for version 1.
